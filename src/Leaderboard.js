...
class Leaderboard extends React.Component {
    ...
    componentDidMount() {
        const accumulatedTotals = this.state.results.reduce(
        ...
        for (var id in accumulatedTotals) {
            this.state.leaderboard.push(
                {
                    movie: this.state.results.find(element => element.imdbID == id),
                    frequency: accumulatedTotals[id]
                },
            )
        }

        this.state.leaderboard.sort((a, b) => (b.frequency - a.frequency))
